...
stores, the rows the mention aggregate asserts, and the digest every current
renderer already knows how to print.

Three adapters, one direction. Storage is not migrated by this module — the
...
Two rules run through all three functions:
...
def mention_rows(record: dict[str, Any]) -> list[tuple[str, str, str]]:
    ...
    What the three columns cannot carry, they drop: an alternative route, an
    applicability condition and a negative polarity all project as the plain
    statement label here. The archived record and the spec's per-claim table
    keep them.
    """
...
def _facts(facts: dict[str, Any]) -> dict[str, Any]:
    ...
    Only `parsed` derivations are reported: `present_unparsed` means the
    document said something the grammar could not read, and the honest summary
    of that is silence, not a number. Money keeps its exact decimal string —
    rounding a salary into v1's integer would be a loss no renderer asked for.
    """
